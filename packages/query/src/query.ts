...
function nested(obj: any, dotProperty: string, value?: any): any | undefined {
  if (!dotProperty) {
    return value || obj;
  }
...
export class Query {
  ...
  withinPolygon(field: string, coords: number[][]) {
    if (Array.isArray(coords) === false || coords.length === 0 || coords[0].length > 3) {
      throw new Error('coords must be a [[number, number]]');
    }

    const withinCoords = coords.map((coord) => {
      if (!isNumber(coord[0]) || !isNumber(coord[1])) {
        throw new Error('coords argument must be a [number, number]');
      }
  ...
  ascending(field: string) {
    if (!isString(field)) {
      throw new Error('The provided field must be a string.');
    }
  ...
  addFilter(field: string, ...args: any) {
    if (!isString(field)) {
      throw new Error('The field argument must be a string.');
    }

    const { condition, values } =
      args.length === 2 ? { condition: args[0], values: args[1] } : { condition: undefined, values: args[0] };
    ...
    if (docs.length > 0) {
      ...
      if (!isEmpty(queryPlainObject.sort)) {
        ...
        processedDocs.sort((a, b) => {
          return Object.keys(queryPlainObject.sort).reduce((result: any, field) => {
      ...
      if (isArray(queryPlainObject.fields) && queryPlainObject.fields.length > 0) {
        processedDocs = processedDocs.map((doc) => {
          ...
          Object.keys(modifiedDoc).forEach((key) => {
            if (
              queryPlainObject.fields &&
              queryPlainObject.fields.indexOf(key) === -1 &&
              PROTECTED_FIELDS.indexOf(key) === -1
            ) {
