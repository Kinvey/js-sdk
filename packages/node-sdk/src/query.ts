...
export class Query {
  ...
  isSupportedOffline(): boolean {
    return Object.keys(this.filter).reduce((supported, key): boolean => {
      if (supported) {
        ...
        return UNSUPPORTED_CONDITIONS.some(
          (unsupportedConditions): boolean => {
            if (!value) {
              return true;
            }
  ...
  process<T extends Doc>(docs: T[] = []): T[] {
    ...
    if (docs.length > 0) {
      ...
      if (isArray(queryObject.fields) && queryObject.fields.length > 0) {
        processedDocs = processedDocs.map(
          (doc): Doc[] => {
            const modifiedDoc: any = doc;
            Object.keys(modifiedDoc).forEach(
              (key): void => {
                if (
                  queryObject.fields &&
                  queryObject.fields.indexOf(key) === -1 &&
  ...
  toHttpQueryObject(): HttpQueryObject {
    ...
    Object.keys(httpQueryObject).forEach(
      (key): any => {
