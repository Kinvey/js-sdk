...
export class Query {
   ...
   */
  constructor(config?: QueryConfig|Query) {
    config = assign({
      fields: [],
      filter: {},
      sort: null,
  ...
  contains(field: string, values: any[]): Query {
    if (isDefined(values) === false) {
      throw new QueryError('You must provide a list of values for Query#contains().');
    }
  ...
  containsAll(field: string, values: any[]): Query {
    if (isDefined(values) === false) {
      throw new QueryError('You must provide a list of values for Query#containsAll().');
    }
  ...
  greaterThan(field: string, value: number|string): Query {
    if (isNumber(value) === false && isString(value) === false) {
      throw new QueryError('You must provide a number or string value for Query#greaterThan().');
    }
   ...
   * @returns {Query} The query.
   */
  private addFilter(field: string, condition: string, values?: any): Query {
    if (isObject(this.filter[field]) === false) {
      this.filter[field] = {};
    }
